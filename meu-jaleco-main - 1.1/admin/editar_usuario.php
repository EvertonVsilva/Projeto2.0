...
if ($method === 'POST') {
    ...
    if ($senha_nova !== '') {
        ...
        if ($s2) {
            ...
            $s2->execute();
            $s2->close();
            $senhaAlteradaPorAdmin = true;
        }
    }


    // Registrar log de edição de dados
    ...
    if ($ins) {
        ...
        $ins->bind_param('is', $id_usuario_logado, $status);
        $ins->execute();
        $ins->close();
    }

    // Se a senha foi alterada, registrar log específico de alteração de senha por admin
    if ($senhaAlteradaPorAdmin) {
        $ins_senha = $mysqli->prepare('INSERT INTO log (id_usuario, data_hora, status_autenticacao) VALUES (?, NOW(), ?)');
        if ($ins_senha) {
            $status_senha = 'SENHA_ALTERADA_ADMIN';
            $ins_senha->bind_param('is', $id_usuario_logado, $status_senha);
            $ins_senha->execute();
            $ins_senha->close();
        }
    }
