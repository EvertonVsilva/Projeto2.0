 ...
 * 
 * Gera usuários fictícios para testes do sistema com dados padronizados.
 * 
 * Lógica de Numeração Automática:
 * - Busca o último usuário teste cadastrado (ex: "Usuario Teste 5")
 * - Extrai o número usando regex
 * - Incrementa para criar o próximo (ex: "Usuario Teste 6")
 * 
 * Dados Gerados:
 * - Nome: "Usuario Teste N"
 * - Email: "usuarioN@teste.com"
 * - Login: "userNN" (user01, user02, etc.) - sempre 6 caracteres
 * - Senha: "teste123" (criptografada com bcrypt)
 * - CPF: Sequência numérica com 11 dígitos (00000000001, 00000000002, etc.)
 * - Telefones: Formato padrão com numeração incremental
 * - Endereço: Dados fictícios mas válidos
 * 
 * Processo:
 * 1. Determina o próximo número de teste
 * 2. Insere endereço primeiro (para obter id_endereco)
 * 3. Insere usuário com referência ao endereço
 ...
 * 
 * Remove um usuário do sistema com deleção em cascata.
 * 
 * Ordem de Deleção (respeitando foreign keys):
 * 1. Logs do usuário (tabela log.id_usuario)
 * 2. Usuário (tabela usuario)
 * 3. Endereço associado (tabela endereco)
 * 
 * Esta ordem é necessária porque:
 * - Logs têm FK para usuario (não pode deletar usuario com logs existentes)
 * - Usuario tem FK para endereco (mas podemos deletar usuario antes)
 * - Endereço pode ser deletado por último pois nenhuma outra tabela depende dele
 * 
 * Importante: Se deletássemos na ordem incorreta, o MySQL retornaria erro de violação de FK.
 */
